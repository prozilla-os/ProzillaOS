import { i as x, m as D } from "/assets/object.utils-xiWGUq8N.js";
const o = {
  /**
   * Foreground colors.
   */
...
const S = /* @__PURE__ */ Symbol("formatContext"), R = /* @__PURE__ */ Symbol.for("react.element"), z = /* @__PURE__ */ Symbol.for("react.fragment"), w = {
  ...
  singleQuotes: !1,
  spaceAfterComma: !0,
  stringColor: o.fg.green,
  numberColor: o.fg.yellow,
  booleanColor: o.fg.yellow,
  nullColor: o.decoration.dim + o.fg.yellow,
  undefinedColor: o.decoration.dim + o.fg.yellow,
  bigintColor: o.fg.yellow,
  symbolColor: o.fg.green,
  functionColor: o.fg.blue,
  keyColor: o.fg.white,
  dateColor: o.fg.magenta,
  regexpColor: o.fg.cyan,
  errorColor: o.fg.red,
  htmlTagColor: o.fg.red,
  reactComponentColor: o.fg.yellow,
  delimiterColor: o.fg.cyan,
  plugins: []
};
function p(e, t) {
  const r = g(e, t);
  if (typeof r == "string")
    return r;
  if (e === null)
    return a("null", r.nullColor, r.colors);
  if (e === void 0)
    return a("undefined", r.undefinedColor, r.colors);
  if (typeof e == "boolean")
    return a(String(e), r.booleanColor, r.colors);
  if (typeof e == "number") {
    let n;
    return Number.isNaN(e) ? n = "NaN" : Number.isFinite(e) ? n = String(e) : n = e > 0 ? "Infinity" : "-Infinity", a(n, r.numberColor, r.colors);
  }
  if (typeof e == "bigint")
    return a(e + "n", r.bigintColor, r.colors);
  if (typeof e == "string")
    return P(e, r);
  if (typeof e == "symbol")
    return a(e.toString(), r.symbolColor, r.colors);
  ...
  if (e instanceof Date)
    return a(e.toISOString(), r.dateColor, r.colors);
  if (e instanceof RegExp)
    return a(e.toString(), r.regexpColor, r.colors);
  ...
  if (e instanceof Promise)
    return a("Promise { <pending> }", r.functionColor, r.colors);
  if (x(e))
    return M(e, r);
  for (const n of r.plugins)
    if (n.fallback) {
      const i = n.fallback(e, r);
      if (i !== void 0)
        return i;
    }
...
function V(e, t, r, n) {
  const i = g(e, { ...n, depth: 3 });
  if (typeof i == "string")
    return i;
  const l = a(e.name || "(anonymous)", i.functionColor, i.colors), c = t.map((y) => p(y, h(i))).join(i.separator()), f = p(r, h(i)), u = i.colors ? s.dim("→") : "→";
  return `${l}(${c}) ${u} ${f}`;
}
function k(e, t) {
  const r = g(e, t);
  ...
  if (typeof e.type == "string")
    n = a(e.type, r.htmlTagColor, r.colors);
  else if (e.type === z)
    n = "", i = !0;
  else {
    if (x(e.type) || typeof e.type == "function") {
      let m = e.type.name;
      "displayName" in e.type && e.type.displayName && (m = e.type.displayName), n = String(m);
    } else
      n = String(e.type);
    n = a(n, r.reactComponentColor, r.colors);
  }
  const l = [];
  let c = [];
  if (e.props && x(e.props)) {
    const m = Object.keys(e.props);
    r.sortKeys && m.sort();
    for (const B of m) {
      const b = e.props[B];
      B === "children" ? c = (Array.isArray(b) ? b : [b]).map((T) => A(T, r)) : l.push(F(B, b, r));
    }
  }
  const f = r.delimiter("<"), u = r.delimiter(">"), y = r.delimiter("/"), d = l.length ? " " + l.join(" ") : "";
  return c.length || i ? `${f}${n}${d}${u}${c.join("")}${f}${y}${n}${u}` : `${f}${n}${d}${y}${u}`;
}
function A(e, t) {
  return e == null || typeof e == "boolean" ? "" : typeof e == "string" ? e : typeof e == "number" ? e.toString() : E(e) ? k(e, h(t)) : "";
}
function F(e, t, r) {
  return `${a(e, r.keyColor, r.colors)}={${p(t, h(r))}}`;
}
function P(e, t) {
  const r = g(e, t);
  if (typeof r == "string")
    return r;
  const n = e.length > r.maxStringLength, i = n ? e.slice(0, r.maxStringLength) : e;
  let l = JSON.stringify(i);
  r.singleQuotes && (l = l.replace(/'/g, "\\'"));
  const c = a(l.slice(1, -1), r.stringColor, r.colors), f = r.delimiter(r.singleQuotes ? "'" : '"'), u = f + c + f;
  return n ? u + a("...", r.nullColor, r.colors) : u;
}
function j(e, t) {
  const r = g(e, t);
  if (typeof r == "string")
  ...
  return a(`[Function: ${n}]`, r.functionColor, r.colors);
}
function I(e, t) {
  const r = g(e, t);
  if (typeof r == "string")
  ...
  if (n !== null)
    return n;
  const i = [], l = Math.min(e.length, r.maxArrayLength);
  for (let c = 0; c < l; c++)
    i.push(p(e[c], h(r)));
  if (e.length > r.maxArrayLength) {
    const c = e.length - r.maxArrayLength;
    i.push(a(`... ${c} more item${c !== 1 ? "s" : ""}`, r.nullColor, r.colors));
  }
  return C(i, r, "[", "]");
}
function M(e, t) {
  ...
  if (n !== null)
    return n;
  let i = Object.keys(e);
  if (r.sortKeys && (i = i.sort()), i.length === 0)
    return r.delimiter("{}");
  const l = i.map((u) => {
    const y = a(u, r.keyColor, r.colors), d = p(e[u], h(r));
    return `${y}${r.delimiter(":")} ${d}`;
  }), c = r.delimiter("{") + " ", f = " " + r.delimiter("}");
  return C(l, r, c, f);
}
function K(e, t) {
  const r = g(e, t);
...
function O(e, t) {
  ...
  for (const l of e)
    i.push(p(l, h(r)));
  return C(i, r, `Set(${e.size}) { `, " }");
}
function Q(e, t) {
  const r = g(e, t);
  if (typeof r == "string")
    return r;
  const n = e.name || "Error", i = e.message || "";
  return a(`${n}: ${i}`, r.errorColor, r.colors);
}
function $(e, t, r) {
  return t.seen.has(e) ? a("[Circular]", t.nullColor, t.colors) : t.currentDepth > t.depth ? a(r, t.nullColor, t.colors) : (t.seen.add(e), null);
}
function C(e, t, r, n) {
  const i = `${r}${e.join(t.separator())}${n}`;
  if (!t.compact && s.strip(i).length > t.breakLength) {
    const l = "	".repeat(t.currentDepth + 1), c = "	".repeat(t.currentDepth);
    return `${r.replace(/ +$/, "")}
${e.map((f) => `${l}${f}`).join(`,
`)}
${c}${n.replace(/^ +/, "")}`;
...
function g(e, t) {
  ...
  t && U(t) ? r = t : r = {
    ...
    delimiter: (n) => a(n, r.delimiterColor, r.colors),
...
function E(e) {
  return x(e) && "$$typeof" in e && e.$$typeof === R;
}
...
class N {
  ...
  statusMessage(t, r, n) {
    ...
    if (r.length) {
      ...
      i += `
` + r.map((c) => l + this.stringify(c)).join(`
`);
    }
...
//# sourceMappingURL=index-DC7HlONQ.js.map
