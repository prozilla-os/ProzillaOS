...
}).setExecute(async function(t, { stdout: a, env: o }) {
  const i = o.exportedVariables;
  if (t.length > 0) {
    await Promise.all(t.map(async (n) => {
      const e = i[n];
      e !== void 0 && await r.printLn(a, `${e}`);
    }));
    return;
  }
  await Promise.all(Object.entries(i).map(async ([n, e]) => {
    await r.printLn(a, `${n}=${e}`);
  }));
});
export {
  l as printenv
};
//# sourceMappingURL=printenv-Cq-skM9d.js.map
