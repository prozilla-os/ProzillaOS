...
export function tokenize(expression: string): Token[] {
	...
	while (i < expression.length) {
		...
		if (THREE_CHAR_OPS.has(s3))  { tokens.push({ type: "op", value: s3 }); i += 3; }
		else if (TWO_CHAR_OPS.has(s2)) { tokens.push({ type: "op", value: s2 }); i += 2; }
		else                           { tokens.push({ type: "op", value: expression[i++] }); }
	}
