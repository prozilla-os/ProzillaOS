import{E as d,n as m,q as l,A as o}from"../index.js";const g=5,f=new d().setRequireArgs(!0).setManual({purpose:"Show system reference manuals",usage:`man [options] page
man -k [options] regexp`,description:`Each page arguments given to man is normally the name of a command.
The manual page associated with this command is then found and displayed.`,options:{"-k":"Search for manual page using regexp"}}).setExecute(function(i,c){const{options:p}=c;if(p!=null&&p.includes("k"))return m.search(i[0].toLowerCase()).map(e=>{var a;return(a=e.manual)!=null&&a.purpose?`${e.name} - ${e.manual.purpose}`:e.name}).sort().join(`
`);const t=i[0].toLowerCase(),u=m.find(t);if(!u)return l(this.name,`${t}: Command not found`);const n=u.manual;if(!n)return l(this.name,`${t}: No manual found`);const r=e=>e.split(`
