import{d as S,b as f,j as r,i as $,s as p}from"../index.js";const E=5,O=new S().setRequireArgs(!0).setManual({purpose:"Show system reference manuals",usage:`man [options] page
man -k [options] regexp`,description:`Each page arguments given to man is normally the name of a command.
The manual page associated with this command is then found and displayed.`,options:{"-k":"Search for manual page using regexp"}}).addOption({short:"k",long:"apropos"}).setExecute(async function(l,{options:w,stdout:c,stderr:d}){if(w.includes("k")){const e=f.builtins.filter(n=>n.name.match(l[0].toLowerCase())).map(n=>n.manual?.purpose?`${n.name} - ${n.manual.purpose}`:n.name).sort().join(`
